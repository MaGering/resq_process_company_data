...
# companies_path = os.path.join(this_path, "results", "companies", "adlershof_companies.csv")
# companies_geodata = os.path.join(this_path, "results", "companies", "adlershof_companies_geodata.csv")
#
# # CSV laden
...
# geocode = RateLimiter(geolocator.geocode, min_delay_seconds=5)  # um Rate Limits zu vermeiden
#
# # Funktion zur Extraktion der Adresse aus der URL
# def extract_address(url):
#     if pd.isna(url):
#         return None
#     # Extrahiere nach '?q=' und dekodiere die URL
#     if "?q=" in url:
#         raw_address = url.split("?q=")[-1]
#         decoded_address = unquote(raw_address)  # <-- Hier passiert die Magie
#         return decoded_address
#     return None
#
# # Funktion zur Umwandlung in Koordinaten
...
#
# # Koordinaten abrufen
# companies[['Latitude', 'Longitude']] = companies["Adresse"].apply(get_coordinates)
#
# # CSV speichern
...
# companies = pd.read_csv(companies_path)
#
# # === Resume progress if existing geodata file ===
# if os.path.exists(companies_geodata):
#     existing = pd.read_csv(companies_geodata)
#     print(f"Resuming from existing geodata file ({len(existing)} entries)...")
#     companies = companies.merge(existing, on=list(companies.columns), how='left', suffixes=('', '_old'))
#     if 'Latitude_old' in companies.columns:
#         companies['Latitude'] = companies['Latitude'].combine_first(companies['Latitude_old'])
#         companies['Longitude'] = companies['Longitude'].combine_first(companies['Longitude_old'])
#         companies.drop(columns=['Latitude_old', 'Longitude_old'], inplace=True)
#
# # === Geocoder setup ===
# geolocator = Nominatim(user_agent="adlershof-geocoder")
...
#
# # === Process ===
# companies["Adresse"] = companies.get("Adresse", companies["Google Maps Link"].apply(extract_address))
#
...
#             print(f"💾 Checkpoint: saving after {counter} processed entries...")
#             companies.to_csv(companies_geodata, index=False)
#
# except KeyboardInterrupt:
#     print("🛑 Interrupted by user. Saving progress before exiting...")
#     companies.to_csv(companies_geodata, index=False)
#     raise
#
